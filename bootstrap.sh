#!/usr/bin/env bash
# Takes a fresh WSL Ubuntu install from nothing to a built home-manager config.
...
if [ -z "$FLAKE_USER" ]; then
  ...
  if [ "$REPLY" = "y" ] || [ "$REPLY" = "Y" ]; then
    # NOTE: Linux (GNU) sed -i takes no '' argument — unlike macOS BSD sed
...
nix run home-manager/release-26.05 -- switch --flake ~/.dotfiles#"$REAL_USER"
